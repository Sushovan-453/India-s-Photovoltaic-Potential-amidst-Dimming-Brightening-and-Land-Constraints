...
CERES_LAND_RAD_all_sky=CERES_LAND_RAD_final;
clear CERES_LAND_RAD_final

cd '/scratch/cas/phd/asz188660/solar_dim_bright/all_dot_mat_file'
load CERES_LAND_RAD_naer_sky_final.mat CERES_LAND_naer_RAD_final
CERES_LAND_RAD_naer_sky=CERES_LAND_naer_RAD_final;
clear CERES_LAND_naer_RAD_final

cd '/scratch/cas/phd/asz188660/solar_dim_bright/all_dot_mat_file'
load CERES_LAND_RAD_clr_sky_final.mat 
CERES_LAND_RAD_clr_sky=CERES_LAND_clr_RAD_final;
clear CERES_LAND_clr_RAD_final

CERES_aerosol_impact_final=CERES_LAND_RAD_naer_sky-CERES_LAND_RAD_all_sky;

cd '/scratch/cas/phd/asz188660/solar_dim_bright/all_dot_mat_file'
load NDVI_GHSL_661_626.mat sara_lat sara_lon ndvi_2015 g_2015

for m=1:18
    ceres_all(:,:,m)=nanmean(CERES_LAND_RAD_all_sky(:,:,(m-1)*12+1:(m*12)),3);
    aerosol_impact(:,:,m)=nanmean(CERES_aerosol_impact_final(:,:,(m-1)*12+1:(m*12)),3);
end

kk_mean(:,:)=nanmean(CERES_LAND_RAD_all_sky,3);
...
%%
for k=1
    b1_total_all_year=aerosol_impact;
...
k3=tren1_total_all_year; clear tren1_total_all_year pval1_total_all_year
k3(isnan(k2))=NaN;
k3(k3<0)=NaN;
figure; geoshow(sara_lat, sara_lon, k3,'DisplayType', 'Surface'); colorbar;
%%
cd '/scratch/cas/phd/asz188660/solar_dim_bright/all_dot_mat_file'
load mask_state_wise.mat maskin
newcell=maskin;
...
%%
figure, set(gcf,'color','w'),axis equal, axis off
  A = [462 933]; I = 380;
  cd '/scratch/cas/phd/asz188660/solar_dim_bright/matlab_code_for_2nd_paper'
